...
const byte SM1_increase_stiffness = 62;
const byte SM1_decrease_stiffness = 63;
const byte SM2_increase_stiffness = 64;
const byte SM2_decrease_stiffness = 65;
...
// Limit Switches
const byte SM1_limit_switch_MIN_stiffness = 29;  // 31
const byte SM1_limit_switch_MAX_stiffness = 10;  // 10
const byte SM2_limit_switch_MIN_stiffness = 13;  // 13
const byte SM2_limit_switch_MAX_stiffness = 31;  // 29
// Fault Pins
...
void setupButtons()
{
  ...
  pinMode(SM1_increase_stiffness, INPUT_PULLUP);
  pinMode(SM1_decrease_stiffness, INPUT_PULLUP);
  pinMode(SM2_increase_stiffness, INPUT_PULLUP);
  pinMode(SM2_decrease_stiffness, INPUT_PULLUP);
  pinMode(ACT_UP_BUTTON, INPUT);
  pinMode(ACT_DOWN_BUTTON, INPUT);
  // Set limit switches as input.
  pinMode(SM1_limit_switch_MIN_stiffness, INPUT_PULLUP);
  pinMode(SM1_limit_switch_MAX_stiffness, INPUT_PULLUP);
  pinMode(SM2_limit_switch_MIN_stiffness, INPUT_PULLUP);
  pinMode(SM2_limit_switch_MAX_stiffness, INPUT_PULLUP);
...
void actionForButton()
{
  ...
  // Check to see if the SM1_increase_stiffness was pressed.
  if (!digitalRead(SM1_increase_stiffness))
  {
    digitalWrite(SM1_DirectionPin, HIGH);
    unsigned long now = millis();
    while (((now + time_for_rotation) > millis()) && (!digitalRead(SM1_limit_switch_MAX_stiffness)))
    {
  ...
  // Check to see if SM1_decrease_stiffness was pressed.
  if (!digitalRead(SM1_decrease_stiffness))
  {
    digitalWrite(SM1_DirectionPin, LOW);
    unsigned long now = millis();
    while (((now + time_for_rotation) > millis()) && (!digitalRead(SM1_limit_switch_MIN_stiffness)))
    {
      Timer1.pwm(SM1_StepPin, 512);
    }
    Timer1.pwm(SM1_StepPin, 0);
  ...
  // Check to see if the SM2_increase_stiffness was pressed.
  if (!digitalRead(SM2_increase_stiffness))
  {
    digitalWrite(SM2_DirectionPin, HIGH);
    unsigned long now = millis();
    while (((now + time_for_rotation) > millis()) && (!digitalRead(SM2_limit_switch_MAX_stiffness)))
    {
  ...
  // Check to see if SM2_decrease_stiffness was pressed.
  if (!digitalRead(SM2_decrease_stiffness))
  {
    digitalWrite(SM2_DirectionPin, LOW);
    unsigned long now = millis();
    while (((now + time_for_rotation) > millis()) && (!digitalRead(SM2_limit_switch_MIN_stiffness)))
    {
      Timer1.pwm(SM2_StepPin, 512);
    }
    Timer1.pwm(SM2_StepPin, 0);
